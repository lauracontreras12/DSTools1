...
min=`echo "$sorted_words" | tail -n 1`
max=`echo "$sorted_words" | head -n 1`
...
float_mean=`echo "$total_freq/$count" | bc -l`
echo "Mean frequency using floating point arithemetics = $float_mean"
